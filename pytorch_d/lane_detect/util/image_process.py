...
from .process_labels import encode_labels, decode_labels, decode_color_labels
...
class LaneDataset(Dataset):
    ...
    def __init__(self, csv_file, transform=None):
        ...
        self.data = pd.read_csv(os.path.join(os.getcwd(), "data_list", csv_file),
                                header=0,
                                names=["image", "label"])
        # 声明数据，地址
...
class ScaleAug(object):
    def __call__(self, sample):
        ...
        if scale < 1.0:  # padding
            ...
            pad_list = [
                [pre_h_pad, h - new_h - pre_h_pad],
                [pre_w_pad, w - new_w - pre_w_pad],
                [0, 0]
            ]
...
def expand_resize_color_data(prediction=None, submission_size=(3384, 1710), offset=690):
    ...
    color_expand_mask = cv2.resize(color_pred_mask,
                                   (submission_size[0], submission_size[1] - offset),
                                   interpolation=cv2.INTER_NEAREST)
