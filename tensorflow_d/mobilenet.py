...
def _depthwise_conv_block(inputs, pointwise_conv_filters, depth_multiplier=1, strides=(1, 1), block_id=1):
    ...
    x = Conv2D(pointwise_conv_filters, (1, 1), padding='same', use_bias=False, name="conv_pw_%d" % block_id)(x)
...
def MobileNet(input_shape=[224, 224, 3],
              ...
              dropout=1e-3,
              classes=1000):
    img_put = Input(shape=input_shape)

    x = _conv_block(img_put, 32, strides=(2, 2))
    ...
    # modle_name = 'mobilenet_1_0_224_tf.h5'
    # model.load_weights(modle_name)
    return model
