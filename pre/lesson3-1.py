...
def inference(w, b, x):  # inference/test/predict/calculate
    pred_y = w * x + b  # (w,b)=theta [b: bias, w: weight]
    return pred_y


def eval_loss(w, b, x_list, gt_y_list):  # gt:global truth
    avg_loss = 0.0
...
def cal_step_gradient(batch_x_list, batch_gt_y_list, w, b, lr):
    ...
    for i in range(batch_size):
        ...
        dw, db = gradient(pred_y, batch_gt_y_list[i], batch_x_list[i])  # 得到导数
        avg_dw += dw
        avg_db += db
    avg_dw /= batch_size
    avg_db /= batch_size
